...
namespace AOC2021._12
{
    public class Day12B : Day
    {
        ...
        public override void Run()
        {
            ...
            FindEnd(start, new Dictionary<string, (Cave cave, int visitCount)>(), start.Name);
            
            Console.WriteLine($"sum = {_count}");
        }
        
        private void FindEnd(Cave c, Dictionary<string, (Cave cave, int visitCount)> visited, string path)
        {
            if (c.IsEnd)
            {
            ...
            {
                visited.Add(c.Name, (c, 1));
            }
            else
            {
                var vc = visited[c.Name];
                vc.visitCount++;
                visited[c.Name] = vc;
            }
            ...
            foreach (var kvp in c.ConnectedCaves)
            {
                ...
                if (v.TryGetValue(kvp.Key, out var cc))
                {
                    if (!cc.cave.IsBig)
                    {
                        ...
                        if (v.Any(a => a.Value.visitCount > 1 && !a.Value.cave.IsBig))
                        {
                            continue;
                        }
