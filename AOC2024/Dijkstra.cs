...
public static class Dijkstra
{
    ...
    public static List<List<(T Node, int Cost)>> GetShortestPaths<T>(T start, T end,
        IEnumerable<T> nodes, Func<T, IEnumerable<(T Neighbour, int Cost)>> neighbourWithCost)
        where T : class, IEquatable<T> 
    {
        var costs = nodes
            .Select(node => (node, Details: (Previous: new List<T>(), Cost: int.MaxValue)))
            .ToDictionary(x => x.node, x => x.Details);
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var path in neighbourWithCost(current))
            {
                ...
                {
                    var (previous, _) = costs[path.Neighbour];
    ...
    private static List<List<(T, int)>> BuildShortestPaths<T>(Dictionary<T, (List<T> Previous, int Cost)> path, List<(T, int)> result, T end)
        where T : class, IEquatable<T>
    {
        var current = end;
        while (true)
        {
            ...
            {
                var results = new List<List<(T, int)>>();
